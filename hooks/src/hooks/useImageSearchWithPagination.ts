...
const useImageSearchWithPagination = (keyword: string) => {
  ...
  const { data, isLoading, error } = useFetch<ImageSearchResult>(`https://pixabay.com/api/?key=${API_KEY}&q=${encodeURIComponent(keyword)}&page=${currentPage}&image_type=photo&pretty=true`);
